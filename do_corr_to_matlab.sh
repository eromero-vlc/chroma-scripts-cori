			...
			for momw in $mom_leaders; do
				...
				for t_source in $prop_t_sources ; do
					for cfg in $confs; do
						...
						[ -f $c ] || continue
						cfg_number="$(
							cat $table | while read c t ch ; do
								[ $c == $cfg -a $t == $t_source ] && echo $ch
							done
						)"
						(
								...
								echo "$redstar_3pt_snkmom_srcmom" | while read snk_src_mom ; do
									...
									echo "$redstar_insertion_disps" | while read disp_label disp ; do 
										...
										for tsep in $gprop_t_seps ; do
											...
											[ -f $f ] || echo $t_source $cfg_number $f
											[ -f $f ] || continue
											echo -n "v.('${momi}_${opi}_${rowi}_${momk}_${insop}_${rowk}_${momj}_${opj}_${rowj}').('tsep${tsep}').('d${disp}')($cfg_number,:)=[" >> ${data}_${t_source}
											awk '
												BEGIN { l=0; }
