...
tagcnf="n$max_nvec"
origconfspath="/global/project/projectdirs/m4155/b6p5"
confspath="/global/cscratch1/sd/eromero/b6p5"
chroma="/global/project/projectdirs/hadron/chromaform0/install-knl/chroma2-mgproto-qphix-avx512/bin/chroma"
chromaform=/project/projectdirs/hadron/chromaform0
chroma=$chromaform/install-knl/chroma-mgproto-qphix-avx512/bin/chroma
...
N_COLOR_FILES=1
if [ "X${zphase}X" != XX ]; then
python $chroma_python/unsmeared_hadron_node.py  -c ${cfg} -e ${ensemble} -g ${lime_file_pre} -n ${nvec} -f ${N_COLOR_FILES} -v ${colorvec_file_pre} -t ${t_offset} -k ${t_seps_commas} -p ${gprop_file_prefix} -d "${GDM}" -s MG -a UNSMEARED_HADRON_NODE_DISTILLATION_SUPERB -M ${MG_PARAM_FILE} -i QPHIX-MG --genprop5 --phase "0.00 0.00 $zphase"  --max-rhs 16 --multiple-writers > $runpath/gprop_creation_${t_source}.xml
else
python $chroma_python/unsmeared_hadron_node.py  -c ${cfg} -e ${ensemble} -g ${lime_file_pre} -n ${nvec} -f ${N_COLOR_FILES} -v ${colorvec_file_pre} -t ${t_offset} -k ${t_seps_commas} -p ${gprop_file_prefix} -d "${GDM}" -s MG -a UNSMEARED_HADRON_NODE_DISTILLATION_SUPERB -M ${MG_PARAM_FILE} -i QPHIX-MG --genprop5 --max-rhs 16 --multiple-writers > $runpath/gprop_creation_${t_source}.xml
fi
...
echo srun -n384 -c68 -N96 \$MY_OFFSET --cpu_bind=cores $chroma -by 4 -bz 4 -pxy 0 -pxyz 0 -c 64 -sy 1 -sz 1 -minct 1 -poolsize 1 -i $runpath/gprop_creation_${t_source}.xml  -geom 3 4 4 $LT &> $runpath/gprop_create_run_${t_source}.out
srun -n384 -c68 -N96 \$MY_OFFSET --cpu_bind=cores $chroma -by 4 -bz 4 -pxy 0 -pxyz 0 -c 64 -sy 1 -sz 1 -minct 1 -poolsize 1 -i $runpath/gprop_creation_${t_source}.xml  -geom 3 4 4 $LT &>> $runpath/gprop_create_run_${t_source}.out
rename _outof_6 _outof_8 ${gprop_file}.*
EOF
